...
resource "libvirt_volume" "extra_disks" {
  ...
  name   = "${each.value.vm_name}-${each.value.name}.qcow2"
  pool   = "default"
  format = "qcow2"
...
dynamic "disk" {
  for_each = {
    for k, v in libvirt_volume.extra_disks :
    k => v
...
resource "null_resource" "ansible_inventory" {
  triggers = {
    content = sha1(templatefile("${path.module}/templates/ssh_config.tpl", {
  ...
  provisioner "local-exec" {
    command = <<EOT
cat <<EOF > ${path.root}/ssh.config
${templatefile("${path.module}/templates/ssh_config.tpl", {
  domains = libvirt_domain.domain_ubuntu
  vms     = var.vms
})}
EOF
